...
| `IUIService` | `UIService` | `PushAsync<T>()` · `CloseAsync()` · `BringToFrontAsync<T>()` · `GetUI<T>()` · `DestroyAsync<T>()` |
| `ISoundService` | `SoundService` | `PlaySound(clip, category)` · `PlaySound3D(clip, pos)` · `PlayMusic(clip)` · `SetMixerVolume(param, vol)` · `GetMixerVolume(param)` · `TransitionToSnapshot(snap)` · `DuckBGM(duration)` · `UnduckBGM()` · `MusicVolume` · `CanPlaySound(data)` · `GetEffectiveVolume(data, vol)` |
