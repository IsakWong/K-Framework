...
public class AssetManager : KSingleton<AssetManager>, IAssetService
{
    ...
    public T LoadAssetReference<T>(AssetReference assetReference) where T : Object
    {
        ...
        return ThrowSyncUnsupported<T>(assetReference);
#else
        // 检查是否已经加载
        if (assetReference.OperationHandle.IsValid() && assetReference.OperationHandle.IsDone)
        {
...
#elif UNITY_WEBGL
        // WebGL 无"同步实例化"语义，请改用异步版本
        return ThrowSyncUnsupported<GameObject>($"{assetReference} (同步实例化，请改用 InstantiateAssetReferenceAsync)");
#else
    ...
    /// <summary>
    /// WebGL 单线程无法同步等待 Addressables 加载。同步 API 仅允许读取
    /// 已在启动/关卡 Loading 阶段用 LoadAssetAsync* 预热（加载完成）的缓存；
    /// 未命中时抛出明确异常，提示把该资源加入预热清单。
    /// </summary>
