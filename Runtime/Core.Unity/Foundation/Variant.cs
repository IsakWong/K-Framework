...
public class Variant : ISerializationCallbackReceiver, IVariant
{
    ...
    private void Deserialize<T>()
    {
        try
        {
            mValue = JsonConvert.DeserializeObject<T>(mValueString);
        }
    ...
    public static implicit operator GameObject(Variant t)
    {
        return t.mObject as GameObject;
    }

    public static implicit operator float(Variant t)
    {
        return (float)t.mValue;
    }

    public static implicit operator int(Variant t)
    {
        return (int)t.mValue;
    }

    public static implicit operator Vector2(Variant t)
    {
        return (Vector2)t.mValue;
    }

    public static implicit operator Vector3(Variant t)
    {
        return (Vector3)t.mValue;
    }

    public static implicit operator Quaternion(Variant t)
    {
        return (Quaternion)t.mValue;
    }
...
/// <summary>
/// Variant 引用，他不会构造Varaint，而是使用外部引用
/// </summary>
/// <typeparam name="T"></typeparam>
public class VariantRef<T> : IVariant
{
    public Variant varaint = new();

    public VariantRef(T t = default)
    {
        varaint = new Variant();
        varaint.Set(t);
    }
